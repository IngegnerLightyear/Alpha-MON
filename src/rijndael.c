...
void init_rijndael_OP(rijndael * self){
	...
	memcpy( self->rcon, rcon, sizeof(rcon) );
};
...
/*void init_rijndael_OP(rijndael self)
{
	self.rijndael_init=rijndael_init;
	self.blockEncrypt=blockEncrypt;
	self.padEncrypt=padEncrypt;
	self.keySched=keySched;
	self.keyEncToDec=keyEncToDec;
	self.r_encrypt=r_encrypt;
	self.r_decrypt=r_decrypt;
}*/
...
int rijndael_init(rijndael * self, Mode mode,Direction dir,const UINT8 * key,KeyLength keyLen,UINT8 * initVector)
{
        ...
        if(initVector)
        {
                ...
                {
                        self->m_initVector[i] = initVector[i];
                }
        } else {
                /* zero init vector */
                for(i = 0;i < MAX_IV_SIZE;i++)
                {
                        self->m_initVector[i] = 0;
                }
        }
	...
	*/
        ...
        keySched(self, keyMatrix);

        if(self->m_direction == Decrypt) keyEncToDec(self);
...
int blockEncrypt(rijndael * self, const UINT8 *input,int inputLen,UINT8 *outBuffer)
{
        ...
        UINT32 *miv = (UINT32 *)self->m_initVector;

        if(self->m_state != Valid)return RIJNDAEL_NOT_INITIALIZED;
        if(self->m_direction != Encrypt)return RIJNDAEL_BAD_DIRECTION;
        ...
        switch(self->m_mode){
                case ECB:
                        ...
                        {
                                r_encrypt(self, input,outBuffer);
                                input +=16;
                                input32 += 4;
                                outBuffer += 16;
                        }
                ...
                case CBC:
                        ...
                        r_encrypt(self, (UINT8 *)block,outBuffer);
                        input += 16;
                ...
                case CFB1:
                        ...
                        for(i = numBlocks; i > 0; i--)
                        {
                                for(k = 0; k < 128; k++)
                                {
                                        ...
                                        block[2] = iv[2];
                                        block[3] = iv[3];
                                        r_encrypt(self, (UINT8 *)block,(UINT8 *)block);
...
int padEncrypt(rijndael * self, const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
        ...
        if(self->m_state != Valid)return RIJNDAEL_NOT_INITIALIZED;
        if(self->m_direction != Encrypt)return RIJNDAEL_NOT_INITIALIZED;
        ...
        switch(self->m_mode)
        {
                case ECB:
                        ...
                        rte_memcpy(block, input, 16 - padLen);
                        memset(block + 4 - padLen, padLen, padLen);
                        r_encrypt(self, (UINT8 *)block,outBuffer);
                        break;
                case CBC:
                        iv = (UINT32 *)self->m_initVector;
                        for(i = numBlocks; i > 0; i--)
                        {
...
int blockDecrypt(rijndael * self, const UINT8 *input, int inputLen, UINT8 *outBuffer)
{
        ...
        miv = (UINT32 *)self->m_initVector;
        input32 = (UINT32 *)input;


        if(self->m_state != Valid)return RIJNDAEL_NOT_INITIALIZED;
        if((self->m_mode != CFB1) && (self->m_direction == Encrypt))return RIJNDAEL_BAD_DIRECTION;
        ...
        switch(self->m_mode)
        {
                case ECB:
                        for (i = numBlocks; i > 0; i--)
                        {
                                r_decrypt(self, input,outBuffer);
                                input += 16;
                                input32 += 4;
                                outBuffer += 16;
                        }
                ...
                case CBC:
                        ...
                        for (i = numBlocks; i > 0; i--)
                        {
                                r_decrypt(self, input, (UINT8 *)block);
                                block[0] ^= iv[0];
                                block[1] ^= iv[1];
                                block[2] ^= iv[2];
                                block[3] ^= iv[3];
#if STRICT_ALIGN
                                rte_memcpy(iv, input, 16);
                                rte_memcpy(outBuf, block, 16);
#else
iv[0] = input32[0];
...
int padDecrypt(rijndael * self, const UINT8 *input, int inputOctets, UINT8 *outBuffer)
{
        ...
        if(self->m_state != Valid)return RIJNDAEL_NOT_INITIALIZED;
        if(self->m_direction != Decrypt)return RIJNDAEL_BAD_DIRECTION;
        ...
        switch(self->m_mode){
                case ECB:
                        ...
                        rte_memcpy(outBuffer, blockb, 16 - padLen);
                        break;
                case CBC:
                        rte_memcpy(iv, self->m_initVector, 16);
                        /* all blocks but last */
                        for (i = numBlocks - 1; i > 0; i--)
                        {
                                r_decrypt(self, input, blockb);
...
void keySched(rijndael * self, UINT8 key[_MAX_KEY_COLUMNS][4])
{
        ...
        int uKeyColumns = self->m_uRounds - 6;
        ...
        for(j = 0;(j < uKeyColumns) && (r <= (int)self->m_uRounds); )
        {
                for(;(j < uKeyColumns) && (t < 4); j++, t++)
                {
                        *((UINT32*)self->m_expandedKey[r][t]) = *((UINT32*)tempKey[j]);
                }
        ...
        while(r <= (int)self->m_uRounds)
        {
                tempKey[0][0] ^= self->S[tempKey[uKeyColumns-1][1]];
                tempKey[0][1] ^= self->S[tempKey[uKeyColumns-1][2]];
                tempKey[0][2] ^= self->S[tempKey[uKeyColumns-1][3]];
                tempKey[0][3] ^= self->S[tempKey[uKeyColumns-1][0]];
                tempKey[0][0] ^= self->rcon[rconpointer++];
                ...
                if (uKeyColumns != 8)
                {
                        ...
                        tempKey[uKeyColumns/2][0] ^= self->S[tempKey[uKeyColumns/2 - 1][0]];
                        tempKey[uKeyColumns/2][1] ^= self->S[tempKey[uKeyColumns/2 - 1][1]];
                        tempKey[uKeyColumns/2][2] ^= self->S[tempKey[uKeyColumns/2 - 1][2]];
                        tempKey[uKeyColumns/2][3] ^= self->S[tempKey[uKeyColumns/2 - 1][3]];
...
void keyEncToDec(rijndael * self)
{
        ...
        for(r = 1; r < (int)self->m_uRounds; r++)
        {
                w = self->m_expandedKey[r][0];
                *((UINT32*)w) = *((UINT32*)self->U1[w[0]]) ^ *((UINT32*)self->U2[w[1]]) ^ *((UINT32*)self->U3[w[2]]) ^ *((UINT32*)self->U4[w[3]]);
                w = self->m_expandedKey[r][1];
                *((UINT32*)w) = *((UINT32*)self->U1[w[0]]) ^ *((UINT32*)self->U2[w[1]]) ^ *((UINT32*)self->U3[w[2]]) ^ *((UINT32*)self->U4[w[3]]);
                w = self->m_expandedKey[r][2];
                *((UINT32*)w) = *((UINT32*)self->U1[w[0]]) ^ *((UINT32*)self->U2[w[1]]) ^ *((UINT32*)self->U3[w[2]]) ^ *((UINT32*)self->U4[w[3]]);
                w = self->m_expandedKey[r][3];
                *((UINT32*)w) = *((UINT32*)self->U1[w[0]]) ^ *((UINT32*)self->U2[w[1]]) ^ *((UINT32*)self->U3[w[2]]) ^ *((UINT32*)self->U4[w[3]]);
        }
}	

static inline UINT32 encrypt_b_from_T(rijndael * self, UINT8 ind1, UINT8 ind2, UINT8 ind3, 
		UINT8 ind4) {

	UINT32 *tmp1 = (UINT32 *)(self->T1[ind1]);
	UINT32 *tmp2 = (UINT32 *)(self->T2[ind2]);
	UINT32 *tmp3 = (UINT32 *)(self->T3[ind3]);
	UINT32 *tmp4 = (UINT32 *)(self->T4[ind4]);

	return *tmp1 ^ *tmp2 ^ *tmp3 ^ *tmp4;
}

static inline UINT32 decrypt_b_from_T(rijndael * self, UINT8 ind1, UINT8 ind2, UINT8 ind3, 
		UINT8 ind4) {

	UINT32 *tmp1 = (UINT32 *)(self->T5[ind1]);
	UINT32 *tmp2 = (UINT32 *)(self->T6[ind2]);
	UINT32 *tmp3 = (UINT32 *)(self->T7[ind3]);
	UINT32 *tmp4 = (UINT32 *)(self->T8[ind4]);
...
void r_encrypt(rijndael * self, const UINT8 a[16], UINT8 b[16])
{
	...
	mkey = (UINT32 *)self->m_expandedKey[0][0];
	temp[0] = a32[0] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[0][1];
	temp[1] = a32[1] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[0][2];
	temp[2] = a32[2] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[0][3];
	temp[3] = a32[3] ^ *mkey;

	tempb = (UINT8 *)temp;
	b32[0] = encrypt_b_from_T(self, tempb[0], tempb[5], tempb[10], tempb[15]);
	b32[1] = encrypt_b_from_T(self, tempb[4], tempb[9], tempb[14], tempb[3]);
	b32[2] = encrypt_b_from_T(self, tempb[8], tempb[13], tempb[2], tempb[7]);
	b32[3] = encrypt_b_from_T(self, tempb[12], tempb[1], tempb[6], tempb[11]);
	
        for(r = 1; r < (int)self->m_uRounds-1; r++)
        {
		mkey = (UINT32 *)self->m_expandedKey[r][0];
                temp[0] = b32[0] ^ *mkey;
		mkey = (UINT32 *)self->m_expandedKey[r][1];
                temp[1] = b32[1] ^ *mkey;
		mkey = (UINT32 *)self->m_expandedKey[r][2];
                temp[2] = b32[2] ^ *mkey;
		mkey = (UINT32 *)self->m_expandedKey[r][3];
                temp[3] = b32[3] ^ *mkey;
	...
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds - 1][0];
	temp[0] = b32[0] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds - 1][1];
	temp[1] = b32[1] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds - 1][2];
	temp[2] = b32[2] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds - 1][3];
	temp[3] = b32[3] ^ *mkey;
        
	tempb = (UINT8 *)temp;
        b[ 0] = self->T1[tempb[0]][1];
        b[ 1] = self->T1[tempb[5]][1];
        b[ 2] = self->T1[tempb[10]][1];
        b[ 3] = self->T1[tempb[15]][1];
        b[ 4] = self->T1[tempb[4]][1];
        b[ 5] = self->T1[tempb[9]][1];
        b[ 6] = self->T1[tempb[14]][1];
        b[ 7] = self->T1[tempb[3]][1];
        b[ 8] = self->T1[tempb[8]][1];
        b[ 9] = self->T1[tempb[13]][1];
        b[10] = self->T1[tempb[2]][1];
        b[11] = self->T1[tempb[7]][1];
        b[12] = self->T1[tempb[12]][1];
        b[13] = self->T1[tempb[1]][1];
        b[14] = self->T1[tempb[6]][1];
        b[15] = self->T1[tempb[11]][1];
	
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds][0];
	b32[0] ^= *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds][1];
	b32[1] ^= *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds][2];
	b32[2] ^= *mkey;
	mkey = (UINT32 *)self->m_expandedKey[self->m_uRounds][3];
	b32[3] ^= *mkey;
...
void r_decrypt(rijndael * self, const UINT8 a[16], UINT8 b[16])
{
	...
	tempb = (UINT8 *)temp;
	b32[0] = decrypt_b_from_T(self, tempb[0], tempb[13], tempb[10], tempb[7]);
	b32[1] = decrypt_b_from_T(self, tempb[4], tempb[1], tempb[14], tempb[11]);
	b32[2] = decrypt_b_from_T(self, tempb[8], tempb[5], tempb[2], tempb[15]);
	b32[3] = decrypt_b_from_T(self, tempb[12], tempb[9], tempb[6], tempb[3]);

        for(r = self->m_uRounds-1; r > 1; r--)
        {
		mkey = (UINT32 *)self->m_expandedKey[r][0];
                temp[0] = b32[0] ^ *mkey;
		mkey = (UINT32 *)self->m_expandedKey[r][1];
	...
	mkey = (UINT32 *)self->m_expandedKey[1][0];
	temp[0] = b32[0] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[1][1];
	temp[1] = b32[1] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[1][2];
	temp[2] = b32[2] ^ *mkey;
	mkey = (UINT32 *)self->m_expandedKey[1][3];
	temp[3] = b32[3] ^ *mkey;

	tempb = (UINT8 *)temp;
        b[ 0] = self->S5[tempb[0]];
        b[ 1] = self->S5[tempb[13]];
        b[ 2] = self->S5[tempb[10]];
        b[ 3] = self->S5[tempb[7]];
        b[ 4] = self->S5[tempb[4]];
        b[ 5] = self->S5[tempb[1]];
        b[ 6] = self->S5[tempb[14]];
        b[ 7] = self->S5[tempb[11]];
        b[ 8] = self->S5[tempb[8]];
        b[ 9] = self->S5[tempb[5]];
        b[10] = self->S5[tempb[2]];
        b[11] = self->S5[tempb[15]];
        b[12] = self->S5[tempb[12]];
        b[13] = self->S5[tempb[9]];
        b[14] = self->S5[tempb[6]];
        b[15] = self->S5[tempb[3]];
